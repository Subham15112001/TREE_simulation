...
let treeContainer;// svg container
let arrayContainer;// svg container
let start;
...
function Tree() {
  ...
  this.createBinaryTree = function(arr) {
    treeContainer = createContainer("binary-tree", arr);//return the box in which graph will be present
...
// create array of rectangles below the graph
function createArray(arr, x, y, width, height) {
  var arrayData = arr.map((value, i) => {
    if (i > 0) {
      x += 50//shift x co ordinate by 50 for each element
    }
    return {// return a object with following attribute
      x: x,
      y: y,
      width: width,
      height: height,
...
// create a line with point (x1,y1) && (x2,y2)
...
//returning a object with dimension
...
//create a container for the given id to be projected
